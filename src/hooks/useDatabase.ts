      ...
      if (notifications.length === 0) {
        setNotifications([
          ...
          },
          {
            ...
            type: "info",
            title: "نصيحة",
            message: "قم بتوثيق حسابك للحصول على مزايا إضافية",
  ...
  const initializeUserBalance = async (userId: string) => {
    try {
      ...
      if (balanceError && balanceError.code !== "PGRST116") {
        ...
        console.log("⚠️ Setting local fallback balance due to error");
        setBalance({
          dzd: 15100, // Welcome bonus included
          eur: 75,
          usd: 85,
          gbp: 65.5,
      ...
      if (!existingBalance) {
        ...
        // Create balance with welcome bonus as per database schema
        const initialBalance = {
          dzd: 15100, // 15000 base + 100 welcome bonus
          eur: 75,
          usd: 85,
          gbp: 65.5,
          investment_balance: 0,
      ...
      // Set welcome balance on any error for better UX
      console.log("⚠️ Setting welcome balance due to unexpected error");
      setBalance({
        dzd: 15100,
        eur: 75,
        usd: 85,
